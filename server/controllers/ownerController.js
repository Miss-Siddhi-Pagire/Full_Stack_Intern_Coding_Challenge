...
const getStoreStats = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data: { storeName: store.name, averageRating: stats.averageRating || 0, totalRatings: stats.totalRatings }
    });
  } catch (err) { next(err); }
};
...
const getAnalytics = async (req, res, next) => {
  try {
    ...
    const store = await Store.findByOwnerId(ownerId);
    if (!store) return res.status(404).json({ success: false, message: 'Store not found' });
    const trends = await Rating.getTrends(store.id);
    const distribution = await Rating.getDistribution(store.id);
